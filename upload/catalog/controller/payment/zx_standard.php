...
class ControllerPaymentZXStandard extends Controller {
    protected function index() {
        ...
        $this->data['business'] = $this->config->get('zx_standard_email');
        ...
        $this->data['notify_url'] = HTTP_SERVER . 'index.php?route=payment/zx_standard/callback';
        $this->data['email'] = $order_info['email'];
        $this->data['invoice'] = $this->session->data['order_id'] . ' - ' . html_entity_decode($order_info['payment_firstname'], ENT_QUOTES, 'UTF-8') . ' ' . html_entity_decode($order_info['payment_lastname'], ENT_QUOTES, 'UTF-8');
        $this->data['lc'] = $this->session->data['language'];

        if (!$this->config->get('zx_standard_transaction')) {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/zx_standard.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/zx_standard.tpl';
        } else {
            $this->template = 'default/template/payment/zx_standard.tpl';
        }
